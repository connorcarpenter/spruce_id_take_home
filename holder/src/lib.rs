...
use shared::{HolderChallengeRequest, HolderVerifyRequest, VerifierChallengeResponse, HolderRegisterRequest, VerifierRegisterResponse, UserId, PublicKey};
...
impl Holder {
    ...
    pub fn recv_challenge_response(&mut self, response: VerifierChallengeResponse) -> Result<HolderVerifyRequest, HolderError> {
        ...
        let signature = self.key_pair.sign(nonce).as_ref().try_into().expect("This should never fail, because ring::signature::Signature is always 64 bytes.");

        Ok(HolderVerifyRequest::new(user_id, signature))
    }
}
